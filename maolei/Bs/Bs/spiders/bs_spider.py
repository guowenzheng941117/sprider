...
class BsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        yield scrapy.Request("http://cse.csu.edu.cn/jyjx/yjsjy/tzgg.htm",
                             callback=self.parse)

    def parse(self, response):
        link = response.xpath(
            "//div[@class='article-list-left-li-l']//a/@href").get()
        detail = response.xpath(
            "//div[@class='article-list-left-li-l']//a/text()").get()
        print(link)
        print(detail)
    ...
    def mail(self):
        ret = True
        try:
            msg = MIMEText('http://cse.csu.edu.cn/jyjx/yjsjy/tzgg.htm',
                           'plain', 'utf-8')
